...
num_classes = 15
# The source dataset also contains class 15 (container-crane). Ignore it to
# retain the official DOTA1.0 class set, IDs 0-14.
ignore_class_ids = [15]
metainfo = dict(
    classes=(
...
# AdamW LR scaled from RTMDet's batch-8 DOTA setting to effective batch 36.
base_lr = 0.001125
...
head_channels = 256
work_dir = 'work_dirs/dota10_yolov10-h_l_640_80e_rand_b4x3_accum3_fp32'
...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_bbox=dict(
            type='mmdet.GIoULoss', loss_weight=loss_bbox_weight)),
    train_cfg=dict(
        assigner=dict(
...
# VOCMetric consumes the in-memory HBB ground truth supplied by the dataset,
# so no COCO JSON conversion is needed. The primary metric is AP at IoU 0.5.
val_evaluator = dict(
...
# =============================== Optimization ===============================
# Epoch-based scheduling keeps warmup/cosine aligned when micro-batch changes.
param_scheduler = [
    dict(
        type='LinearLR',
